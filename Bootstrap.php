...
class Bootstrap
{
    ...
    public static function navs($options = array(), $htmlOptions = array())
    {
        $htmlOptions = self::addCssClass('nav', $htmlOptions);
        $htmlOptions = self::repValue('class', 'type', self::NAV_TABS, $htmlOptions, 'nav-');
        ...
        foreach ($options as $row) 
        {
            ...
            if(isset($row['active']))
            {
                $liOptions = self::addCssClass('active');
                unset($row['active']);
            }
            if(isset($row['disabled']))
            {
                $liOptions = self::addCssClass('disabled', $liOptions);
                unset($row['disabled']);
            }


            $html .= self::openTag('li', $liOptions);
            $label = self::getValue($row, 'label');
            $url = self::getValue($row, 'url', '#');
            $icon = self::getValue($row, 'icon') !== '' ? self::icon(self::getValue($row, 'icon')) .' ' : '';
            $html .= self::tag('a', array('href' => $url), $icon . $label);
    ...
    {
        $openTagOptions = self::addCssClass('btn-group');
        $menuOption = array();
        if(self::getValue($htmlOptions, 'menuOption', false))
        {   
            $menuOption = self::getValue($htmlOptions, 'menuOption', array());
            unset($htmlOptions['menuOption']);
        }

        if(self::getValue($htmlOptions, 'dropup', false))
        {
            unset($htmlOptions['dropup']);
            $openTagOptions = self::addCssClass('dropup', $openTagOptions);
        }

        $html = self::openTag('div', $openTagOptions);
        if(self::getValue($htmlOptions, 'split', false))
        {
            unset($htmlOptions['split']);
            $html .= self::button($label, $htmlOptions);
            $htmlOptions = self::addCssClass('dropdown-toggle', $htmlOptions);
            $htmlOptions = self::addAttributes('data-toggle', 'dropdown', $htmlOptions);
    ...
    {
        $htmlOptions = self::popValue('class', 'pull', $htmlOptions, 'pull-');
    ...
    {
        $htmlOptions = self::addCssClass('btn', $htmlOptions);
        ...
        $htmlOptions = self::popValue('href', 'url', $htmlOptions);
    ...
    public static function getValue($array, $attr, $ifNull = '')
    {
